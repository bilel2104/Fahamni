...
var User = require('../model/appUserFahamniModel');
...
exports.list_all_pics = function(req, res) {
  User.getAllpics(req.params.matiere, function(err, User) {
    if (err)
      res.send(err);
    res.json(User);
  });
...
exports.read_a_User = function(req, res) {
  User.getUserById(req.params.email,req.params.password, function(err, User) {
    if (err)
      res.send(err);
    res.json(User[0]);
  });
